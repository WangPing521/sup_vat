...
writer1 = SummaryWriter("sup_vat_loss")
...
class CoTrainer(Trainer):
    ...
    def _train_loop(self,
        ...
        for batch_num in n_batch_iter:
            ...
            supervisedLoss, vat_loss, jsdLoss, advLoss = 0, 0, 0,0
            ...
            vat_loss = VATLoss()
            vat_reg1, _, _ = vat_loss(self.segmentators[0], unlab_img)
            vat_reg2, _, _ = vat_loss(self.segmentators[1], unlab_img)
            vat_loss = vat_reg1 + vat_reg2
            # ============================
            if train_jsd:
            ...
            totalLoss = supervisedLoss + vat_loss
        ...
        writer1.add_scalars('vat_sup_loss',
                            {'loss_sup': supervisedLoss.item() / batch_num, 'vat_loss': vat_loss.item() / batch_num,
                             'total_loss': totalLoss.item() / batch_num}, epoch)
    ...
    def _eval_loop(self,
        ...
        for batch_num, [(img, gt), _, path] in enumerate(val_dataloader):
            ...
            dsc_dict = {f"S{i}": {f"DSC{n}": batchdiceMeters[i].value()[1][0][n] for n in self.axises} for i in
                        range(S)}
